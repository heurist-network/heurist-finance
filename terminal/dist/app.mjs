...
function renderPlainMacro(pillars, width, title) {
  ...
  for (const p of pillars) {
    ...
    const bar = c(barColor, "\u2588".repeat(filled)) + pc("muted", "\u2591".repeat(empty));
    const state = p.stateLabel || "\u2014";
    const dir = p.direction || "";
    const arrow = directionArrow(dir);
    const stateStr = padRight(c(barColor, state), 14);
...
function earningsSurprise(opts = {}) {
  ...
  for (let i = 0; i < quarters.length; i++) {
    ...
    if (isForward) {
      ...
      const yoy = q.yearAgoEps != null ? padLeft(c(palette("positive"), `+${((q.estimate / q.yearAgoEps - 1) * 100).toFixed(0)}% YoY`), pctW + 3) : padLeft(pc("muted", ""), pctW);
...
function renderChartPanel(data, width) {
  const { values, volume, label, height, showAxis } = data;
  if (!values?.length) return EARLY(pc("muted", "\u2014 No chart data"));
  return brailleChart({ values, width, height, showAxis, volume, label });
}
...
function buildFooter(width) {
  ...
  const meta = [
    m.model ? `${DIM2}${m.model}${RESET2}` : null,
    m.tools ? `${DIM2}${m.tools} tools${RESET2}` : null,
    costStr,
...
function saveReport() {
  ...
  try {
    fs2.mkdirSync(REPORTS_DIR, { recursive: true });
    ...
    fs2.writeFileSync(
      path3.join(REPORTS_DIR, filename),
...
function listReports() {
  try {
    ...
    return fs2.readdirSync(REPORTS_DIR).filter((f) => f.endsWith(".json")).sort().reverse().slice(0, 20);
  } catch {
    return [];
  }
}
...
var VERSION_CACHE = path4.join(os3.homedir(), ".heurist", "version.json");
...
function readCache() {
  try {
    const raw = fs3.readFileSync(VERSION_CACHE, "utf8");
...
function writeCache(latest) {
  try {
    fs3.mkdirSync(path4.dirname(VERSION_CACHE), { recursive: true });
    fs3.writeFileSync(VERSION_CACHE, JSON.stringify({
      latest,
      checkedAt: Date.now()
    }));
  } catch {
...
function handleKeypress(ch, key) {
  ...
  if (num >= 1 && num <= 9 && tui.agentState?.stage === "complete" && tui.agentState?.follow_ups?.length) {
  ...
  if (ch === "q") {
    ...
    tui.splashMsg = `Waiting for agent \xB7 ${hint}`;
    startSplashAnimation();
    return;
  }
  if (key.name === "up" || ch === "k") {
